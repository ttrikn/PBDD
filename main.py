...
class Trainer(object):
    def __init__(self, args):
        ...
        self.test_set = MSADataset(args, processors[args.dataset], tsv_file='test01.tsv', max_seq_length=64 if args.dataset=='masad' else 128)
...
#         print("=====")
#         print(args)
#         print(self.test_set.label_id_list)
#         print("=====")
        self.model.to(self.device)
    ...
    def save(self):
        ...
        with open(os.path.join('D://PyCharm/FS1/UP-MPF-main/output/', str(self.best_ckpt['time']).replace(':', '_')) + '.txt', 'w', encoding='utf-8') as f:
            f.write('#True\t#Pred\n')
    ...
    def save_best_ckpt_params(self):
        best_ckpt_params_path = os.path.join(self.save_dir, 'best_ckpt_params.ckpt')
        torch.save(self.best_ckpt['embedding'], best_ckpt_params_path)
        print(f"[#] Best checkpoint parameters saved to {best_ckpt_params_path}.")
    ...
    def load_best_ckpt_params(self):
        best_ckpt_params_path = os.path.join("D://PyCharm/best_ckpt_params.ckpt")
        if os.path.exists(best_ckpt_params_path):
            self.best_ckpt['embedding'] = torch.load(best_ckpt_params_path)
            print(f"[#] Best checkpoint parameters loaded from {best_ckpt_params_path}.")
        else:
            print("[#] No saved best checkpoint parameters found.")
    ...
    def evaluate_on_test(self):
        ...
        self.save()
        self.save_best_ckpt_params()
        # token_y = self.test_set.tokenizer.convert_ids_to_tokens(test_y)
        # token_y_ = self.test_set.tokenizer.convert_ids_to_tokens(test_y_)
        # num_samples = len(token_y_)
        # window_size = 10
        # threshold = 5
        # correct_predictions = 0
        # for i in range(0, num_samples, window_size):
        #     true_window = token_y[i:i + window_size]
        #     predicted_window = token_y_[i:i + window_size]
        #
        #     common_labels = set(true_window).intersection(predicted_window)
        #     common_count = len(common_labels)
        #
        #     if common_count >= threshold:
        #         correct_predictions += 1
        #
        # accuracy = correct_predictions / (num_samples / window_size)
        # print(f"Accuracy: {accuracy * 100:.2f}%")

    def evaluate_on_demo(self):
        print('[#] Begin to evaluate on test set')
        self.load_best_ckpt_params()
        self.model.load_state_dict(self.best_ckpt["embedding"])
        _, test_acc, test_f1_mac, test_f1_wtd, test_y, test_y_ = self._evaluate('Test')
        token_y = self.test_set.tokenizer.convert_ids_to_tokens(test_y)
        token_y_ = self.test_set.tokenizer.convert_ids_to_tokens(test_y_)
        num_samples = len(token_y)
        window_size = 10
        threshold = 4
        correct_predictions = 0
        ...
        for i in range(0, num_samples, window_size):
            ...
            common_count = sum(1 for true, pred in zip(true_window, predicted_window) if true == pred)
